...
class FarmDetailsActivity : DaggerAppCompatActivity(), OnMapReadyCallback {
    ...
    private fun initViews() {
        val mapFragment =
            supportFragmentManager.findFragmentById(R.id.farmMapView) as SupportMapFragment
        ...
        farm.farmLocation.latitude.forEachIndexed { index, _ ->
            val latLng =
                LatLng(farm.farmLocation.latitude[index], farm.farmLocation.longitude[index])
            latLngs.add(latLng)
        }
        latLngCache = latLngs
